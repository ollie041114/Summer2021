...
contract UNICOIN is ERC20 {
  ...
  function increaseBalanceOf(address _owner, uint256 _amount) private{
    //balances[_owner] = balances[_owner] + _amount;
    balances[_owner] = SafeMath.add(balances[_owner], _amount);
  }
  
  function decreaseBalanceOf(address _owner, uint256 _amount) private{
    //balances[_owner] = balances[_owner] - _amount;
    balances[_owner] = SafeMath.sub(balances[_owner], _amount);
  }
  
  function increaseCirculation(address _owner, uint256 _amount) public{
    _totalSupply = SafeMath.add(_totalSupply, _amount);
    increaseBalanceOf(_owner, _amount);
  }
  
  function decreaseCirculation(address _owner, uint256 _amount) public{
    _totalSupply = _totalSupply - _amount;
    decreaseBalanceOf(_owner, _amount);
  }
  
  function enroll(address _owner, uint256 _amount) public {
    balances[_owner] = SafeMath.add(balances[_owner], _amount);
  }
